...
public class Placeholder {
    ...
    public enum JobsPlaceHolders {
        ...
        user_archived_jobs,
        user_jobs,
        user_dailyquests_pending_name_$1("questNumber"),
        user_dailyquests_pending_description_$1("questNumber"),
        user_dailyquests_pending_progress_$1("questNumber"),
        user_dailyquests_pending_goal_$1("questNumber"),
        user_dailyquests_pending_objectives_$1("questNumber"),
        ...
        if (user != null) {
            ...
            if (placeHolder.isComplex()) {
                ...
                switch (placeHolder) {
                    ...
                    case user_dailyquests_pending_name_$1:
                        try {
                            List<QuestProgression> pending = user.getQuestProgressions().stream().filter(questProg -> !questProg.isCompleted()).collect(Collectors.toList());
                            QuestProgression questProgression = pending.get(Integer.parseInt(vals.get(0)));
                            return questProgression.getQuest().getQuestName();
                        } catch (Exception ex) {
                            return "";
                        }
                    case user_dailyquests_pending_description_$1:
                        try {
                            List<QuestProgression> pending = user.getQuestProgressions().stream().filter(questProg -> !questProg.isCompleted()).collect(Collectors.toList());
                            QuestProgression questProgression = pending.get(Integer.parseInt(vals.get(0)));
                            return CMIChatColor.translate(CMIList.listToString(questProgression.getQuest().getDescription(), "\n"));
                        } catch (Exception ex) {
                            return "";
                        }
                    case user_dailyquests_pending_progress_$1:
                        try {
                            List<QuestProgression> pending = user.getQuestProgressions().stream().filter(questProg -> !questProg.isCompleted()).collect(Collectors.toList());
                            QuestProgression questProgression = pending.get(Integer.parseInt(vals.get(0)));
                    ...
                    case user_dailyquests_pending_goal_$1:
                        try {
                            List<QuestProgression> pending = user.getQuestProgressions().stream().filter(questProg -> !questProg.isCompleted()).collect(Collectors.toList());
                            QuestProgression questProgression = pending.get(Integer.parseInt(vals.get(0)));
                            return questProgression.getTotalAmountNeeded() + "";
                        } catch (Exception ex) {
                            return "";
                        }
                    case user_dailyquests_pending_objectives_$1:
                        try {
                            List<QuestProgression> pending = user.getQuestProgressions().stream().filter(questProg -> !questProg.isCompleted()).collect(Collectors.toList());
                            QuestProgression questProgression = pending.get(Integer.parseInt(vals.get(0)));
                            List<String> objectives = new ArrayList<>();
                            for (java.util.Map<String, QuestObjective> oneAction : questProgression.getQuest().getObjectives().values()) {
                                for (Map.Entry<String, QuestObjective> oneObjective : oneAction.entrySet()) {
                                    objectives.add(Jobs.getLanguage().getMessage("command.info.output." + oneObjective.getValue().getAction().toString().toLowerCase() + ".info") + " " +
                                            ...
                                            + " " + questProgression.getAmountDone(oneObjective.getValue()) + "/"
                                            + oneObjective.getValue().getAmount());
                                }
                            }
                            return CMIChatColor.translate(CMIList.listToString(objectives, "\n"));
                        } catch (Exception ex) {
                            return "";
                        }
                    default:
